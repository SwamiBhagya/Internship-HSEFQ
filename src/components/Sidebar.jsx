...
// ✅ Correct
import { SIDEBAR_MENU } from "../data/sidebarData";

const Sidebar = ({ isOpen, onClose }) => {
  const location = useLocation();
  const isActive = (path) => location.pathname === path;

  // Safety Check: If data didn't load, show an error instead of crashing
  if (!SIDEBAR_MENU) {
    return <div className="text-red-500 p-4">Error: Menu Data Missing</div>;
  }
  ...
  return (
    ...
    >
        ...
        {SIDEBAR_MENU.map((item) => {
          ...
          if (item.type === "special") {
            return (
              ...
              >
                <div
                  className={`p-1.5 rounded-lg ${isActive(item.path) ? "bg-blue-500 text-white" : "bg-blue-900/50 text-blue-400"}`}
                >
          ...
          if (item.type === "disabled") {
            return (
              <div
                key={item.id}
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-700 cursor-not-allowed hover:bg-slate-800/20 opacity-60"
              >
      ...
      <div className="p-4 border-t border-slate-800">
        ...
        <p className="text-center text-[10px] text-slate-600 mt-4 opacity-50">
          v1.1.0 • Data Driven
        </p>
      </div>
    </aside>
...
const SidebarLink = ({ to, icon, label, active, onClick }) => (
  ...
  >
    <span
      className={`text-lg ${active ? "text-white" : "text-slate-500 group-hover:text-white"}`}
    >
      {icon}
    </span>
