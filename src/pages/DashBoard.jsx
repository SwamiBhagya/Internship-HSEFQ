...
import { FaExclamationTriangle, FaCheckCircle, FaClipboardList, FaUsers, FaPlus, FaArrowRight } from 'react-icons/fa';
...
const Dashboard = () => {
  ...
  const recentIncidents = [
    ...
    { id: "INC-004", title: "Server Room Overheating", category: "Safety", status: "Open", severity: "Critical" }, // Added one more for demo
  ];
    ...
    <div className="space-y-6 md:space-y-8 animate-fadeIn pb-20 md:pb-0">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-2 md:px-0 mx-10 my-10">
        <div>
          ...
          <p className="text-gray-500 text-xs md:text-sm mt-1">Here is what's happening at the facility today.</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex justify-between items-center">
          <h2 className="font-bold text-gray-800 text-base md:text-lg flex items-center gap-2">
            Recent Incidents
            <span className="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full hidden sm:inline-block">Last 24h</span>
          </h2>
          ...
          <table className="w-full text-left border-collapse min-w-[600px] md:min-w-full">
            ...
            <tbody className="text-xs md:text-sm divide-y divide-gray-50">
              {recentIncidents.map((item) => (
                <tr key={item.id} className="hover:bg-blue-50/30 transition-colors cursor-default">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border ${
                      item.severity === 'Critical' ? 'bg-red-100 text-red-700 border-red-200' :
                      item.severity === 'High' ? 'bg-orange-100 text-orange-700 border-orange-200' : 
                      item.severity === 'Medium' ? 'bg-yellow-100 text-yellow-700 border-yellow-200' : 
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${item.status === 'Resolved' ? 'bg-green-500' : 'bg-blue-500 animate-pulse'}`} />
                      <span className={`font-medium ${item.status === 'Resolved' ? 'text-green-700' : 'text-blue-700'}`}>
                        {item.status}
                      </span>
                    </div>
                  </td>
      ...
      <Link to="/report" className="md:hidden fixed bottom-6 right-6 bg-blue-600 text-white p-4 rounded-full shadow-lg shadow-blue-600/30 hover:bg-blue-700 active:scale-90 transition-transform z-50">
        <FaPlus size={24} />
      </Link>
...
const StatCard = ({ icon, title, value, bgColor, alert }) => (
  <div className="bg-white p-4 md:p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between h-32 md:h-auto relative overflow-hidden group">
    ...
    <div className={`absolute -right-4 -top-4 w-16 h-16 rounded-full opacity-20 ${bgColor} group-hover:scale-150 transition-transform duration-500`} />
    ...
    <div>
      ...
      <p className="text-[11px] md:text-sm text-gray-500 font-semibold uppercase tracking-wide">{title}</p>
    </div>
  </div>
);
